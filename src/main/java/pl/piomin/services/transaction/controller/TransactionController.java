...
/*
    ...
    public String performTransaction(@RequestBody TransactionRequest request) throws Exception {
    	...
    	System.out.println("accounts.getAccounts().get(request.getFromId()) = "+accounts.getAccounts().get(request.getFromId()));
    	Admin web3j_1 = Admin.build(new HttpService());
    	PersonalUnlockAccount personalUnlockAccount = web3j_1.personalUnlockAccount(accounts.getAccounts().get(request.getFromId()),"123456").send();
    	System.out.println("From personalUnlockAccount.hasError() = "+personalUnlockAccount.hasError());
